    ...
    return (
      <div className="usa-width-one-whole bidder-manager-bidders ab-lower-section">
        <div className="ab-table-title-row">
          <h3>{this.state.CDOView ? 'Internal CDO View' : 'External Bureau View'}</h3>
        ...
        <table className="bidder-manager-bidders">
              ...
              <th className="action-header">
                <div className="bureau-view-toggle">
                  <ToggleButton
                    ...
                    checked={!this.state.CDOView}
                    onChange={() => this.setState({ CDOView: !this.state.CDOView })}
                    onColor="#888"
                  />
                </div>
              </th>
          ...
          <tbody>
            {[...Array(10).keys()].map((m) => {
              return (<AvailableBidderRow
                id={shortid.generate()}
                bidder={bidder}
                isLoading={biddersIsLoading}
                showModal={() => this.showModal()}
                CDOView={this.state.CDOView}
              />);
            })}
          </tbody>
        </table>
