...
const BidderPortfolioStatRow = ({ userProfile, showEdit, classifications }) => {
  ...
  const orgShortDesc = get(
    userProfile,
    "current_assignment.position.organization"
  );
  ...
  return (
    <div className="usa-grid-full bidder-portfolio-stat-row">
      ...
      <div className="bidder-portfolio-ribbon-container">
        <div className="ribbon-container-condensed">{ribbon()}</div>
      </div>
      <div>
        <div>
          ...
          <div className="stat-card-data-point">
            <dt>Location (Org):</dt>
            <dd>
              {currentAssignmentText || NO_POST} ({orgShortDesc})
            </dd>
          </div>
        </div>
        {!showEdit && (
      ...
      {showSaveAndCancel && (
        <div className="stat-card-btn-container">
          <button
            className="stat-card-cancel-btn"
            onClick={() => setEdit(false)}
          >
            Cancel
          </button>
          <button onClick={saveEdit} disabled={!comments && !altEmail}>
            Save
          </button>
        </div>
      )}
