...
export function cyclePositionsFiltersHasErrored(state = false, action) {
  switch (action.type) {
    case 'CYCLE_POSITIONS_FILTERS_HAS_ERRORED':
      return action.hasErrored;
    default:
      return state;
  }
}
export function cyclePositionsFiltersIsLoading(state = false, action) {
  switch (action.type) {
