...
export const mapBidData = (data = []) => {
  ...
  data$ = data$.map(m => {
    ...
    if (m[hs] === HAND_SHAKE_OFFERED_PROP && m[status] === SUBMITTED_PROP) {
      override[status] = HAND_SHAKE_OFFERED_PROP;
    }
    // Handshake submitted and handshake offered
    if (m[hs] === HAND_SHAKE_OFFERED_PROP && m[status] === HAND_SHAKE_NEEDS_REGISTER_PROP) {
      override[status] = HAND_SHAKE_OFFERED_PROP;
    }
    // Handshake accepted, allow bidder to decline
    if (m[hs] === HAND_SHAKE_OFFER_ACCEPTED_PROP && m[status] === SUBMITTED_PROP) {
      override[status] = HAND_SHAKE_OFFERED_PROP;
    ...
    if (m[hs] === HAND_SHAKE_OFFER_ACCEPTED_PROP && m[status] === HAND_SHAKE_NEEDS_REGISTER_PROP) {
    ...
    if (m[hs] === HAND_SHAKE_OFFER_DECLINED_PROP && m[status] === SUBMITTED_PROP) {
      override[status] = HAND_SHAKE_OFFERED_PROP;
    ...
    if (m[hs] === HAND_SHAKE_OFFER_DECLINED_PROP && m[status] === HAND_SHAKE_NEEDS_REGISTER_PROP) {
      override[status] = HAND_SHAKE_OFFERED_PROP;
      // Use a new OFFER_DECLINED status, clone the tracker status of HAND_SHAKE_OFFERED_PROP for
      // this and use a prop to show the offer has already been declined,
      // and allow the bidder/CDO to accept
    ...
    // Handshake revoked
    if (m[hs] === HAND_SHAKE_REVOKED_PROP) {
